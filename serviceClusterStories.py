...
def getEntitiesAndEnrichSources(sources, paramsDict):
	...
	try:
		...
		serverOn = nlpIsServerOn(corenlpServerHost)
...
def writeGraph(settings, sources):
	...
	try:
		outputPath = '/data/graphs/' + settings['name'] + '/'
...
if __name__ == "__main__":
	...
	if( len(sys.argv) < 2 ):
		...
		corenlpServerHost = sys.argv[1]
		
		while( True ):

			allParameters = getConfigParameters( '/data/generic/serviceClusterStories.config.json' )
			prevNow = datetime.now()
			
			for job in allParameters['jobs']:
				try:
					childConfigPath = '/data/graph-cursors/' + job['config']
